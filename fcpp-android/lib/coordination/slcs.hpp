...
//! @brief Export list for SLCS logic formulas.
using slcs_t = export_list<bool, abf_hops_t>;
//! @brief Storage list for SLCS logic formulas.
using slcs_s = storage_list<tags::diameter, hops_t>;
...
FUN bool F(ARGS, bool f) { CODE
    return abf_hops(CALL, f) < node.storage(tags::diameter{});
}
